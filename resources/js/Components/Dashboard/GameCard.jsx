...
export default function GameCard({ game, isLive = false }) {
    ...
    // Determina se deve mostrar o botão externo
    const showOfficialBtn = league?.id === 'nba' && (() => {
        const statusStr = typeof status === 'string' ? status : (status?.long || status?.state || '');
        return /live|in progress|playing|halftime/i.test(statusStr);
    })();

    return (
        <div>
            <Link href={`/games/${game.league?.id || 'nba'}/${game.id}`}>
                <div className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-900/90 via-gray-800/90 to-gray-900/90 backdrop-blur-sm p-5 shadow-xl border border-white/5 hover:border-orange-500/30 hover:shadow-orange-500/10 hover:shadow-2xl transition-all duration-300 cursor-pointer hover:scale-[1.02] hover:-translate-y-0.5">
                    {/* Premium Glow Effect */}
            ...
            {/* Botão externo para NBA */}
            {showOfficialBtn && (
                <a
                    href="https://www.nba.com/watch/featured"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-2 inline-flex items-center gap-2 rounded-xl bg-blue-700 px-6 py-3 font-bold text-white shadow-md hover:bg-blue-800 transition-all"
                >
