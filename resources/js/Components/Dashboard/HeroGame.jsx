...
export default function HeroGame({ game }) {
        if (typeof window !== 'undefined') {
            console.log('HeroGame status:', game?.status);
        }
    if (!game) {
        return (
    ...
    return (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900/95 via-gray-800/95 to-gray-900/95 backdrop-blur-xl shadow-2xl border border-white/5 transition-all duration-300">
            ...
            <div className="relative p-8 md:p-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-8 mb-8">
                    {/* Home Team */}
                    <div className="text-center md:text-right transition-all duration-300">
                ...
                {/* Action Button */}
                <div className="text-center transition-all duration-300 flex flex-col items-center gap-4">
                    <Link
                    ...
                    {/* External NBA button: only when game is live */}
                    {game.league?.id === 'nba' && (() => {
                        const statusStr = typeof game.status === 'string' ? game.status : (game.status?.long || game.status?.state || '');
                        if (/live|in progress|playing|halftime/i.test(statusStr)) {
                            return (
                                <a
                                    href="https://www.nba.com/watch/featured"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center gap-2 rounded-xl bg-blue-700 px-6 py-3 font-bold text-white shadow-md hover:bg-blue-800 transition-all"
                                >
                                    <Play className="w-5 h-5" fill="white" />
                                    <span>Watch on official site</span>
                                </a>
                            );
                        }
                        return null;
